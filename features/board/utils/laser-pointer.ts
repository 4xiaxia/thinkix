...
export class LaserPointer {
  ...
  public init(board: PlaitBoard): void {
    this.boardContainer = PlaitBoard.getBoardContainer(board);
    this.container = this.boardContainer.closest('.board-wrapper') as HTMLElement;
    
    if (!this.container) {
      this.container = this.boardContainer;
    }

    let canvas = this.container.querySelector(`.${LASER_POINTER_CLASS_NAME}`) as HTMLCanvasElement;
    ...
    this.mouseMoveHandler = (event: PointerEvent) => {
  ...
  public destroy(): void {
    ...
    if (this.cvsDom && this.cvsDom.parentNode) {
      this.cvsDom.parentNode.removeChild(this.cvsDom);
    }
  ...
  private draw(): void {
    ...
    if (this.mouseTrack.length >= 3) {
      ...
      this.ctx.save();
      this.ctx.beginPath();
      this.ctx.fillStyle = `rgba(255, 80, 80, 0.65)`;
